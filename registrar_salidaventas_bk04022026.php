...
function ajaxRazonSocial(f){
	...
	//Vista Cliente 1 Interno 2 WS Externo
	var vistaCliente=document.getElementById("vistaCliente").value;
	ajax=nuevoAjax();
	...
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			...
			if(vistaCliente==1){ //SI LA VISTA CLIENTE ES 1 BUSCAMOS SI ES 2 WS NO HACEMOS NADA
				ajaxClienteBuscar();
...
// Variable de Configuración para Formulario Psicotrópicos
var config_piscotropico = <?=obtenerValorConfiguracion($enlaceCon, 61) ?? '2'?>; // Formulario Psicotrópico-> 1: Valida, 2: Omite
...
function validar(f){	
	...
	console.log("TIPO DE DOCUMENTO: " + $("#tipo_documento").val());
	if($("#nitCliente").val()=="0"){
		Swal.fire("Nit!", "Se requiere un número de NIT / CI / CEX válido", "warning");
		return(false);
	}
	
	/*if( $("#tipo_documento").val()==1 && $("#nitCliente").val().length>9 ){
		Swal.fire("CI / NIT!", "El Nro. de CI debe estar entre 1 y 9 caracteres", "warning");
		return(false);
	}
	*/
				...
				confirmButtonText: 'Si, Vender/Facturar!'
				    ...
				    if (result.value) {
				      console.log("Enviando");
						...
						// Verificación de Productos Controlados
						if(verificaProductoControlado() && config_piscotropico == 1){
...
// Función para validar los campos antes de enviar el formulario completo
...
/**
 * Verificación de ITEMS de Producto_Controlado
 */
function verificaProductoControlado(){
    let found = false;
    $('input[id^="producto_controlado"]').each(function() {
        if ($(this).val() == '1') {
            found = true;
    ...
    // if (found) {
    //     console.log("Se encontró un producto controlado con valor 1.");
    // } else {
    //     console.log("No se encontró ningún producto controlado con valor 1.");
    // }
}
  ...
  if(nomcli==""||nit==""|| (mail==""&&tel1=="")){
    Swal.fire("Informativo!", "Debe llenar los campos obligatorios", "warning");
  	...
  	// }else{
		    ...
		    var parametros={"nomcli":nomcli,"propietario":propietario,"nit":nit,"ci":ci,"dir":dir,"tel1":tel1,"mail":mail,"area":area,"fact":fact,"edad":edad,"propietario":propietario,"tipoPrecio":tipoPrecio,"genero":genero,"dv":1};
		    $.ajax({
		        type: "GET",
		        dataType: 'html',
  ...
  if(nomcli==""||nit==""||mail==""||tel1==""){
    Swal.fire("Informativo!", "Debe llenar los campos obligatorios", "warning");
  	...
  	if(validarCorreoUnicoCliente(cod_cliente,nit,mail)==0){
		    ...
		    var parametros={"nomcli":nomcli,"nit":nit,"ci":ci,"dir":dir,"tel1":tel1,"mail":mail,"area":area,"fact":fact,"edad":edad,"propietario":propietario,"genero":genero,"dv":1,"cod_cliente":cod_cliente};
		    $.ajax({
		        type: "GET",
		        dataType: 'html',
...
<nav class="mb-4 navbar navbar-expand-lg" style='background:#006db3 !important;color:white !important;'>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent-4">
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item active">
                            <a href="#" onclick="guardarPedido(0)" class="btn btn-danger btn-fab float-right" style="background:orange;color:#fff;" title="GUARDAR VENTA PERDIDA" data-toggle='tooltip'><i class="material-icons">rule</i></a>
                        </li>  
...
<th colspan='2' width="15%">Cliente/Paciente</th>
</tr>
<tr>
						...
						if ($response !== false) {
						    ...
						    if (json_last_error() === JSON_ERROR_NONE) {
						        foreach ($data as $paciente) {
						            ?>
						            <option value="<?=$paciente['codigo'];?>"><?=$paciente['nombre_completo'];?></option>
						            <?php
						        }
						    } 
						} else {
				...
				<tr>
					...
					<td align="center" width="8%">
						<div id="idprecio<?=$nro_materialActivo?>">
							<link href="stilos.css" rel="stylesheet" type="text/css">
							<input type="hidden" value="1000" name="<?=$globalAlmacen?>" id="<?=$globalAlmacen?>">
							<input type="number" id="precio_unitario<?=$nro_materialActivo?>" min="0.01" name="precio_unitario<?=$nro_materialActivo?>" value="<?=$cotizacion_precio_unitario?>" class="inputnumber" step="0.01" onkeyup="calculaMontoMaterial(<?=$nro_materialActivo?>);">
...
<div id="divRecuadroExt" style="background-color:#666; position:absolute; width:1300px; height: 550px; top:30px; left:50px; visibility: hidden; opacity: .70; -moz-opacity: .70; filter:alpha(opacity=70); -webkit-border-radius: 20px; -moz-border-radius: 20px; z-index:2; overflow: auto;">
</div>

<div id="divboton" style="position: absolute; top:20px; left:1310px;visibility:hidden; text-align:center; z-index:3">
	<a href="javascript:Hidden();"><img src="imagenes/cerrar4.png" height="45px" width="45px"></a>
</div>

<div id="divProfileData" style="background-color:#FFF; width:1250px; height:500px; position:absolute; top:50px; left:70px; -webkit-border-radius: 20px; 	-moz-border-radius: 20px; visibility: hidden; z-index:2; overflow: auto;">
  	<div id="divProfileDetail" style="visibility:hidden; text-align:center">
		<table align='center'>
	     ...
	     <tr>
			...
			<td>
				<div class="row">
					<div class="col-sm-3"><input type='text' placeholder='--' name='itemCodigoMaterial' id='itemCodigoMaterial' class="textogranderojo" onkeypress="return pressEnter(event, this.form);" onkeyup="return pressEnter(event, this.form);"></div>
...
<div class="modal fade modal-primary" id="modalPsicotropico" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-lg">
		<div class="modal-content card">
			<div class="card-header card-header-primary card-header-icon">
				...
				<h4 class="card-title text-primary font-weight-bold">Registro de Productos Controlados</h4>
				<button type="button" class="btn btn-danger btn-sm btn-fab float-right" data-dismiss="modal" aria-hidden="true" style="position:absolute;top:0px;right:0;">
					<i class="material-icons">close</i>
				</button>
			</div>
			<div class="card-body">
				<div class="row p-3">
					<div class="col-md-12">
						<!-- Input para Nro. Receta -->
						<input type="hidden" id="nro_receta" name="nro_receta" value="0">
...
<div class="modal fade modal-primary" id="modalNuevoCliente" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content card" style="background:#1F2E84 !important;color:#fff;">
                ...
                <div class="row">
					<label class="col-sm-2 col-form-label text-white">Propietario</label>
					<div class="col-sm-10">
						<div class="form-group">
						<input class="form-control" style="color:black;background: #fff;text-transform:uppercase;" type="text" id="propietario" value="" required placeholder="Nombre del Propietario" onkeyup="javascript:this.value=this.value.toUpperCase();"/>
						</div>
					</div>
				</div>
 ...
 ?><center><p>Obtenga el CUFD para las ventas del día de HOY en la sucursal.</p>
 		<table class="table table-bordered table-success" style="width: 60%;">
 			...
 			<tr><td><b>FECHA CUFD IMPUESTOS</b></td><td align="left"><?=date("d/m/Y")?></td></tr>
					...
					?><center><a href="siat_folder/siat_cuis_cufd/generar_cufd.php?cod_ciudad=<?=$_COOKIE['global_agencia']?>&cod_entidad=1&l=1" class="btn btn-warning" style="height: 60px;font-size: 20px;">Obtener CUFD <br> <img src="imagenes/actua.gif" width="80" height="80" style="position: absolute;top:0;right:0;"></a></center><?php			
