...
$array_lineas=implode(",",$rptLineas);
...
$txt_reporte="Fecha de Reporte <strong>$fecha_reporte</strong>";
	...
	echo "<table align='center' class='textotit' width='70%'><tr><td align='center'>Reporte Existencias Almacen
		...
		if($rptOrdenar==1){
			if($rpt_ver==1){
				$sql_item="SELECT m.codigo_material, m.descripcion_material, m.cantidad_presentacion from material_apoyo m, proveedores p, proveedores_lineas pl
					where p.cod_proveedor=pl.cod_proveedor and pl.cod_linea_proveedor=m.cod_linea_proveedor and m.codigo_material<>0 and m.estado='1' and pl.cod_linea_proveedor in ($array_lineas) order by m.descripcion_material";
			}else{
				$sql_item="SELECT DISTINCT m.codigo_material, m.descripcion_material, m.cantidad_presentacion from material_apoyo m, proveedores p, proveedores_lineas pl, 
				ingreso_almacenes i, ingreso_detalle_almacenes id
					where i.cod_ingreso_almacen=id.cod_ingreso_almacen and id.cod_material=m.codigo_material and  i.ingreso_anulado=0 and 
					i.cod_almacen='$rpt_almacen' and p.cod_proveedor=pl.cod_proveedor and pl.cod_linea_proveedor=m.cod_linea_proveedor and 
					m.codigo_material<>0 and pl.cod_linea_proveedor in ($array_lineas) and m.estado='1' order by m.descripcion_material";
			}
		}else{
			if($rpt_ver==1){
				...
				material_apoyo m where p.cod_proveedor=pl.cod_proveedor and pl.cod_linea_proveedor=m.cod_linea_proveedor and m.estado='1' and pl.cod_linea_proveedor in ($array_lineas) order by 4,2";
			}else{
				$sql_item="SELECT DISTINCT m.codigo_material, 
				m.descripcion_material, cantidad_presentacion, CONCAT(p.nombre_proveedor,' - ',pl.nombre_linea_proveedor)as linea  from proveedores p, proveedores_lineas pl, 
				material_apoyo m, ingreso_almacenes i, ingreso_detalle_almacenes id where p.cod_proveedor=pl.cod_proveedor and 
				pl.cod_linea_proveedor=m.cod_linea_proveedor and i.cod_ingreso_almacen=id.cod_ingreso_almacen and id.cod_material=m.codigo_material and  
				i.ingreso_anulado=0 and i.cod_almacen='$rpt_almacen' and m.estado='1' and pl.cod_linea_proveedor in ($array_lineas) order by 4,2";
			}			
		}
		...
		{	$codigo_item=$datos_item[0];
			...
			$txtFechaVencimiento=obtenerFechaVencimiento($enlaceCon, $rpt_almacen, $codigo_item);
			...
			if($rptFechaVenc==1){
				$cadena_mostrar.="<td>$txtFechaVencimiento</td>";
			}
