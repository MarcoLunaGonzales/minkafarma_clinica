...
function listaMateriales(f){
	...
	var codigoItem=f.itemCodigoMaterial.value;
	var nombreItem=f.itemNombreMaterial.value;
	contenedor = document.getElementById('divListaMateriales');
	ajax=nuevoAjax();
	ajax.open("GET", "ajaxListaMaterialesIngreso.php?codTipo="+codTipo+"&codigoItem="+codigoItem+"&nombreItem="+nombreItem,true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			contenedor.innerHTML = ajax.responseText;
		}
...
function buscarMaterial(f, numMaterial){
	...
	// document.getElementById('itemTipoMaterial').selectedIndex=proveedor;
	
	console.log('pasoo:'+proveedor);
...
$txtIngresoCajasUnitarios="";
if($banderaIngresoCajas==0){
	$txtIngresoCajasUnitarios="*** El ingreso de los productos se realizara por CAJAS.";
}else{
	$txtIngresoCajasUnitarios="*** El ingreso de los productos se realizara por UNIDADES.";
}
...
echo "<table border='0' class='textotit' align='center'>
		...
		<tr>
			<th align='left'><span class='textopequenorojo' style='background-color:aqua;'><b>$txtIngresoCajasUnitarios</b></span></th>
			<th></th>
			<th align='left'></th>
		</tr>
		</table>";
...
echo "<tr>
	<th>Nro. Ingreso: <b>$nro_correlativo<b>
	<input type='text' disabled='true' class='texto' value='$fecha' id='fecha' size='10' name='fecha'></th>";

echo"<th>Tipo de Ingreso: </td><th>";
...
echo "<th>Nro. Documento:
		<input type='number' class='texto' name='nro_factura' value='' id='nro_factura' required>
	</th>";

echo "<th colspan='1'>Observaciones:</th>
	<th colspan='1'><textarea class='texto' name='observaciones' value='$observaciones' size='20'></textarea></th></tr>";
...
echo "<th colspan='1'>NIT: <input type='number' class='texto' name='nit_proveedor' id='nit_proveedor'></th>
	<th colspan='1'>Días de Credito: <input type='number' class='texto' name='dias_credito' id='dias_credito' min='0' max='180' readonly></th>
	<th colspan='1'>Fecha Documento Proveedor: </th>
	<th><input type='date' class='texto' name='fecha_factura_proveedor' id='fecha_factura_proveedor' required></th>";
...
?>
        <div class="contenedor">
        	<div class="codigo-barras div-center">
				<input class="boton" type="button" value="Nuevo Producto (+)" onclick="mas(this)" accesskey="A"/>
...
<div id="divboton" style="position: absolute; top:20px; left:1170px;visibility:hidden; text-align:center; z-index:3">
...
<div id="divProfileData" style="background-color:#FFF; width:1050px; height:450px; position:absolute; top:50px; left:170px; -webkit-border-radius: 20px; 	-moz-border-radius: 20px; visibility: hidden; z-index:2;">
  	<div id="divProfileDetail" style="visibility:hidden; text-align:center; height:445px; overflow-y: scroll;">
		<table align='center' class="texto">
			<tr><th>Linea</th><th>Codigo</th><th>Producto</th><th>&nbsp;</th></tr>
			<tr>
			<td><select name='itemTipoMaterial' id="itemTipoMaterial" class="textomedianorojo" style="width:300px">
			<?php
			$sqlTipo="SELECT p.cod_proveedor, concat(p.nombre_proveedor, ' (',tp.nombre_tipoventa,')') 
						FROM proveedores p 
						LEFT JOIN tipos_proveedor tp ON tp.cod_tipoventa = p.cod_tipoproveedor
						ORDER BY 2";
			...
			<td>
				<input type='text' name='itemCodigoMaterial' id="itemCodigoMaterial" size="10" class="textogranderojo" onkeypress="return pressEnter(event, this.form);">
			</td>
			<td>
...
<input type='hidden' name='prodItemActivo' value="0">
...
<script>
	...
	$(document).on('change', '#proveedor', function(){
		let cod_proveedor = $(this).val();
		// alert(cod_proveedor)
		$('#itemTipoMaterial').val(cod_proveedor).change(); 
	});
