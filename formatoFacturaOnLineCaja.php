...
<style type="text/css">
	body {color:#000 }
	/* Estilo para el texto de fondo de agua */
  .watermark {
      position: fixed;
      top: 40%;
      left: 42%;
      ...
      font-size: 100px;
      color: rgba(255, 0, 0, 0.8); /* Rojo con transparencia */
...
$tamanoLargo=450+($nroItems*10)-10;
...
<?php
if($estadoSalida==3 && $ventaAnulada==1){
?>
...
<label class="arial-12"><?="NOMBRE/RAZÓN SOCIAL: ".$razonSocialCliente.""?></label><br>
...
<div style="border-bottom: 1px solid black;border-bottom-style: dotted;">
<label class="arial-12"><?="Usuario: $nombreFuncionario"?></label><br>
</div>
<label class="arial-7"><?="Paciente: $txtNombreCliente"?></label>
...
<table>
	<!--tr>
		<td colspan="2" align="center">
			<label class="arial-7" style="align-content: center;">C: <?=$abrevCampaniaHabilitada;?> -> <?=$totalVentaClienteCampania?></label>
		</td>
	</tr-->
...
<script type="text/javascript">
 ...
 // Intentar cerrar la pestaña; si no es posible, redirigir como respaldo
 setTimeout(function () {
	 try {
		 window.open('','_self');
		 window.close();
		 // Si la pestaña no se cerró, redirigir
		 setTimeout(function(){ window.location.href = 'registrar_salidaventas.php'; }, 500);
	 } catch(e) {
		 window.location.href = 'registrar_salidaventas.php';
	 }
 }, 1000);
</script>
