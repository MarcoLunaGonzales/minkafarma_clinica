	...
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			...
			if($("#cliente").val()==146){
				...
				let nitCliente = $('#cliente option:selected').data('nitcliente');
				$('#nitCliente').val(nitCliente);
			}
...
function ajaxClienteBuscar(f){
	...
	var nitCliente=document.getElementById("nitCliente").value;
	ajax=nuevoAjax();
	ajax.open("GET", "ajaxClienteListaPedido.php?nitCliente="+nitCliente,true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
...
function mas(obj) {
	if(num>=1000){
		...
		if(banderaItems0==0){
			...
			ajax=nuevoAjax();
			var cod_precio=0;
			ajax.open("GET","ajaxMaterialPedido.php?codigo="+num+"&cod_precio="+cod_precio,true);
			ajax.onreadystatechange=function(){
				if (ajax.readyState==4) {
  	...
  	if( nomcli==""){
    	...
    	Swal.fire("Informativo!", "Debe llenar el Telefono o Email", "warning");
  		...
  		// }else{
		    ...
		    var parametros={"nomcli":nomcli,"nit":nit,"ci":ci,"dir":dir,"tel1":tel1,"mail":mail,"area":area,"fact":fact,"edad":edad,"propietario":propietario,"tipoPrecio":tipoPrecio,"genero":genero,"dv":1};
		    $.ajax({
		        type: "GET",
		        dataType: 'html',
...
<nav class="mb-4 navbar navbar-expand-lg" style='background:#f0f0f0 !important;color:black !important;'>
                <a class="navbar-brand font-bold" href="#">Fecha de Pedido: [<?php echo $fechaSistemaSesion?>][<b id="hora_sistema"><?php echo $horaSistemaSesion;?></b>] [<?php echo $nombreAlmacenSesion;?>]</a>
...
<form action='guardarPedido.php' method='POST' name='form1' id="guardarSalidaVenta" onsubmit="return validar(this)">
...
<tr class="bg-info text-white" align='center' id='venta_detalle' style="color:#fff;background:#cc6666 !important; font-size: 13px;">
<?php
...
<td align='center'>
	<input type='date' class='texto' value='<?php echo $fecha;?>' id='fecha' size='10' name='fecha' required>
</td>
	...
	<td align='center' id='divCliente' width="20%">	
		<!--span class="input-group-btn" style="position:absolute;width:100px !important;"-->		
			<select name='cliente' class='selectpicker form-control' data-live-search="true" id='cliente' onChange='ajaxRazonSocialCliente(this.form);' required data-style="btn btn-rose">
				...
				<?php
					$sql = "SELECT c.cod_cliente, CONCAT(c.nombre_cliente) as nombre_cliente, c.nit_cliente
							FROM clientes c
					...
					while($rowCot=mysqli_fetch_array($resp)){
						$index++;
				?>
				<option value='<?=$rowCot['cod_cliente']?>' data-nitcliente="<?=$rowCot['nit_cliente']?>"><?=$rowCot['nombre_cliente'];?>[<?=$rowCot['cod_cliente'];?>]</option>
				<?php
					}
				?>
			</select>
...
<?php
	...
	echo "<div class='divBotones2'>
            ...
            <tr>
               <td style='font-size:12px;color:#0691CD; font-weight:bold;' hidden>EFECTIVO Bs.</td>
               <td style='font-size:12px;color:#189B22; font-weight:bold;' class='d-none'>EFECTIVO $ USD</td
            </tr>
             <tr>
               <td><input type='hidden' name='efectivoRecibidoUnido' onChange='aplicarMontoCombinadoEfectivo(form1);' onkeyup='aplicarMontoCombinadoEfectivo(form1);' onkeydown='aplicarMontoCombinadoEfectivo(form1);' id='efectivoRecibidoUnido' style='height:25px;font-size:18px;width:100%;' value='0' step='any'></td>
