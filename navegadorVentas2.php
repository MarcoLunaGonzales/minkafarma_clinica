...
function ajaxBuscarVentas(f){
    var fechaIniBusqueda, fechaFinBusqueda, nroCorrelativoBusqueda, verBusqueda, global_almacen, clienteBusqueda,vendedorBusqueda,tipoVentaBusqueda,clienteBusqueda;
    ...
    location.href="navegadorVentas2.php?fechaIniBusqueda="+fechaIniBusqueda+"&fechaFinBusqueda="+fechaFinBusqueda+"&nroCorrelativoBusqueda="+nroCorrelativoBusqueda+"&vendedorBusqueda="+vendedorBusqueda+"&tipoPagoBusqueda="+tipoVentaBusqueda+"&clienteBusqueda="+clienteBusqueda;
}
...
echo "<tr><th>&nbsp;</th><th>Nro. Doc</th><th>Fecha/hora<br>Registro Salida</th><th>Vendedor</th><th>TipoPago</th>
    <th>Cliente</th><th>NIT</th><th>Monto</th><th>Observaciones</th><th>AsignarMedico</th><th>Imprimir</th><th>Editar</br>DatosVenta</th></tr>";
...
while ($dat = mysqli_fetch_array($resp)) {
    ...
    if ($conf_controlPago == 1 && $codTipoPago == 4 && $montoCancelado < $montoVenta) {
        ...
        $estadoPago = 0; // Cambia el estado de pago a pendiente (0)
        $chk = "<input type='checkbox' name='codigo' value='$codigo'>";
    } elseif($conf_controlPago == 1 && $codTipoPago != 4 ){
        $chk = "<input type='checkbox' name='codigo' value='$codigo'>";
    }
    else{
    ...
    echo "<td>$stikea <span style='color:blue'>$nombreCliente</span><br><small>$razonSocial</smalls> $stikec</td>
    <td>$stikea $nitCli $stikec</td><td>$stikea $montoVentaFormat $stikec</td><td>$stikea $obs_salida $stikec</td>";
    ...
    */
    if($codTipoDoc==1){
        ...
        if($formato_hoja == 1){
            echo "<td  bgcolor='$color_fondo'><a href='formatoNotaRemision.php?codVenta=$codigo' target='_BLANK'><img src='imagenes/factura1.jpg' width='30' border='0' title='Factura Normal'></a></td>";
        }else if($formato_hoja == 2){
            echo "<td  bgcolor='$color_fondo'><a href='formatoNotaRemisionHoja.php?codVenta=$codigo' target='_BLANK'><img src='imagenes/factura1.jpg' width='30' border='0' title='Factura Formato Pequeño'></a>
            </td>";
        }
        // Editar Datos
