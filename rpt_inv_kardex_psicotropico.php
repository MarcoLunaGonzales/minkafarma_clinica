...
<?php
$sql_listaProductos = "SELECT ma.codigo_material, ma.descripcion_material, ma.principio_activo
						FROM material_apoyo ma
						WHERE ma.producto_controlado = 1";
$resp_listaProductos = mysqli_query($enlaceCon, $sql_listaProductos);
while($listaProducto = mysqli_fetch_array($resp_listaProductos)){

	// Codigo de Producto
	$rpt_item = $listaProducto['codigo_material'];
...
$txt_reporte="Fecha de Reporte <strong>$fecha_reporte</strong>";
  ...
  $precioVenta=number_format($precioVenta,2,'.','');
	 ...
	 **/
	$d_emp_regente 				 = '';
	$d_emp_matricula_profesional = '';
	$sql_empresa = "SELECT de.regente, de.matricula_profesional
					FROM datos_empresa de
					LIMIT 1";
	$resp_empresa = mysqli_query($enlaceCon, $sql_empresa);
	if ($resp_empresa) {
		$datos_empresa = mysqli_fetch_assoc($resp_empresa);
		if ($datos_empresa) {
			$d_emp_regente 				 = $datos_empresa['regente'];
			$d_emp_matricula_profesional = $datos_empresa['matricula_profesional'];
		}
	}
	...
	echo "<table align='center' class='textotit'><tr><td align='center'>Reporte Kardex - Psicotrópicos<br>Territorio: 
	<strong>$nombre_territorio</strong> Almacen: <strong>$nombre_almacen</strong><br> Fecha inicio: <strong>$fecha_ini</strong> Fecha final: 
	<strong>$fecha_fin</strong><br>Item: <strong>($rpt_item) $nombre_item</strong><br>$txt_reporte
	<br>Regente Responsable: <strong>$d_emp_regente</strong>
	<br>Matricula Profesional: <strong>$d_emp_matricula_profesional</strong></th></tr></table>";
...
//	echo $sql;
	...
	echo "<tr class='textomini'><th>Proceso</th><th>Tipo</th><th>Nro. Ingreso/Salida</th><th>Fecha</th><th>Factura</th><th>Nro. Receta</th><th>Nombre del Paciente</th><th>Nombre del Médico</th><th>Observaciones<br>/ Razón Social</th><th>Entrada Cajas</th><th>Salida Cajas</th><th>Saldo Cajas</th><th class='bg-plomoclaro'>Entrada Sueltas</th><th class='bg-plomoclaro'>Salida Sueltas</th><th class='bg-plomoclaro'>Saldo Sueltas</th><th>Precio Venta</th><th>Tipo Ingreso/Salida</th><th>Sucursal<br>Origen/Destino</th><th>Nro<br>Ingreso/Salida<br>Destino/Origen</th><th>Responsable</th></tr>";
	...
	{	
      ...
      $razon_social=$dat_salidas['razon_social'];            
			...
			$nro_receta		 = $dat_salidas['nro_receta'];
			$nombre_paciente = $dat_salidas['nombre_paciente'];
			$nombre_medico	 = $dat_salidas['nombre_medico'];
	...
	echo "<tr class='titulos'><th colspan='9'>Sumas</th>
